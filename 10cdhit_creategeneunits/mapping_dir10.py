...
#	samtools index {sample}_md_A_super_v2.bam
	...
	commands = """
	/home/phuong/cd-hit-v4.6.4-2015-0603/cd-hit-est -i {sample}_definedseqs_v3.fa -o {sample}_definedseqs_v4.fa -c .98 -n 10
	...
	java -jar $PICARD_HOME/picard.jar MarkDuplicates INPUT={outfile}.bam OUTPUT={sample}_md_dir10.bam REMOVE_DUPLICATES=TRUE ASSUME_SORTED=TRUE METRICS_FILE={sample}_md_dir10.metrics
	samtools index {sample}_md_dir10.bam
	samtools mpileup -d 1000000 -u -I -t DP -t SP -B -A -f {ref} {sample}_md_dir10.bam | /home/phuong/bcftools-1.3.1/bcftools call -c - > {sample}_dir10.vcf
	samtools mpileup -d 1000000 -u -I -t DP -t SP -B -A -f {ref} {sample}_md_dir10.bam | /home/phuong/bcftools-1.3.1/bcftools call -cv - > {sample}_dir10_vars.vcf
	rm {out_paired}.sam {out_paired}.bam {out_unpaired}.sam {out_unpaired}.bam {sample}.raw.bam
	python makeVCFcov.py {ref} {sample}_dir10.vcf {sample}_dir10.cov
	python get_alternative_alleles.py {sample} {sample}_dir10.vcf
