...
for line in protfile:
	if ">" in line:
		...
		protdict[line.strip()[1:]] = next(protfile).strip()
...
############# save some important information, like gene family, exon number, the number of pieces, and the original assembled contig name
	...
	if not info[0] in fastadict.keys():
		continue
...
############# write out a file and align (via MAFFT) the query sequence (assembled contig) with the target sequence (reference sequence it blasted to)
	...
	cmd = 'mafft --preservecase alignthistemp > outalignedtemp'
	os.system(cmd)
...
############## write out the protein that belongs to the target sequence (because the target sequence is a full locus [exon + intron]) and you want to blast the query sequence (assembled contig) to all proteins that have identical pieces/exon value 
			...
			for key in protdict:
				if '|' + genefamily + '|' in key and '|' + exonnumber + '|' in key and '|' + pieces + '|' in key:
					...
					out.write('>' + key + '\n')
					out.write(protdict[key]+'\n')
				
			out.close()

			myproteinlength = len(protdict[key])
		elif '>' in line:
...
################################### run tblastn, blastx, and exonerate. Pick the one that had at least 70% of the protein blasted/exonerated. If not, just use the 'guess' option -- which was guessed through a mafft alignment
	...
	os.system('tblastn -query mytempprot -db mytempexon -outfmt 6 -out temptblastnoutput -max_target_seqs 1 -max_hsps 1')
	os.system('blastx -query mytempexon -db mytempprot -outfmt 6 -out tempblastxoutput -max_target_seqs 1 -max_hsps 1')
...
######## if less than 70% of the protein was blasted, then a guess is better
	...
	if pickdict[pickthisfile] == 0:
		...
		for i in [0,1,2]:
			if blastedprotein in translate(fastadict[myseqheader][myguessstart+i:myguessend]):

				mypredictedstart = myguessstart + i
				mypredictedend = myguessend
			else:
				continue
		...
		myout.write('>' + '|'.join([myseqheader, genefamily, exonnumber, pieces, str(mypredictedstart), str(mypredictedend), 'guessed']) + '\n')
		myout.write(fastadict[myseqheader]+ '\n')
		myout.write('>' + '|'.join([myseqheader, genefamily, exonnumber, pieces, str(mypredictedstart), str(mypredictedend), 'guessed']) + '\n')
		myout.write(translate(fastadict[myseqheader][mypredictedstart:mypredictedend])+ '\n')
