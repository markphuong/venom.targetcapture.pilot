...
######################## open aligned and find start end for coding regions..
	...
	out.write('\t'.join([ID1, str(fastadict[ID1][locus][1]),str(fastadict[ID1][locus][2]), ID2, str(fastadict[ID2][locus][1]), str(fastadict[ID2][locus][2]), locus, str(pdistcoding[0]), str(pdistcoding[1]), str(pdistnoncoding[0]), str(pdistnoncoding[1]), str(mystart), str(myend)]) + '\n')
