...
variables = dict(
ref = sys.argv[2] + '_definedseqs_v5.fa'
) #name your output
...
myfastafile = open(sys.argv[2] + '_definedseqs_v5.fa', 'r')
...
myprots = open(sys.argv[1] + '_with_utrs.fa', 'r') 
...
########## find first and second exon
			...
			cutcoords = open(sys.argv[1] + '_stats_venomblastcoordfile_utr', 'a')
			coordlist = []
			...
			for line in myblast:
				...
				myrange1 = range(int(info[6])-1,int(info[7]))
...
######## run exonerate, the 1 untranslated sequence to 1 constructed 'genomic' sequence. write out the cutting coordinates
				...
				if len(seq.strip()) == all_qresult[0][0][0].query_span:
					myout = open(sys.argv[1] + '_stats_venombed_utr', 'a')
