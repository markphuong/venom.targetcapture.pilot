...
def align(element):
	...
	commands = """
	makeblastdb -dbtype nucl -in {ID}_haplotypes.fa
	blastn -query unique.mature.toxins.reference.fa -db {ID}_haplotypes.fa -outfmt 6 -out {ID}.expressionblast -evalue 1e-3 -word_size 7 -perc_identity 95
	python get.expression.stats.py {ID}.expressionblast {species} {ID}_venom_analyzethese.fa
	""".format(**variables)
