...
############# extend seed sequence to the left
...
def leftit(sequence, start, end,ID):
	...
	mytrffile = open(SampleID+'_trf_left.output', 'r')
	...
	for line in mytrffile:
		if '@' in line:
			continue
		elif int(line.strip().split(' ')[2]) == 2:
			myleftcounter += float(line.strip().split(' ')[3])


	mytrffile.close()

	return [start, end, myleftcounter]


########### extend read sequence to the right
...
def rightit(sequence, start, end,ID):
	...
	mytrffile = open(SampleID+'_trf_right.output', 'r')
	...
	for line in mytrffile:
		if '@' in line:
			...
			myrightcounter += float(line.strip().split(' ')[3])

	mytrffile.close()

	return [start, end, myrightcounter]


def sam_magic(contigname, mystart, myend, thelength, SampleID, myreadlist):

	samfile = pysam.AlignmentFile(SampleID + "_sorted.bam", "rb")
...
myfasta = open(SampleID + '_assemblies_clustered.fasta.NI', 'r')
...
############### if the reads sequence is below 100bp, try to extend it but avoid extensions that run into simple repeat regions
	if len(fastadict[info[0]][start:end]) < 100:
		...
		if os.path.getsize(SampleID+'_trf.output') == 0:
			...
			myrepeatdict = {'orig' : 0, 'left' : 0, 'right': 0}

			for trfline in mytrffile:
				if '@' in trfline:
					continue
				elif int(trfline.strip().split(' ')[2]) == 2:
					myperiod = int(trfline.strip().split(' ')[2])
					myrepeatdict['orig'] += float(trfline.strip().split(' ')[3])
			mytrffile.close()

############### if it is a dinucleotide in the trf output file, try to extend seed sequences on both ends of the sequence to avoid tandem repeats
############## if both extensions suck, keep the seed sequence that has the minimum amount of tandem repeats
			...
			if myperiod == 2:
				...
				if len(fastadict[info[0]][leftseq[0]:leftseq[1]]) >= 100:
					myrepeatdict['left'] = leftseq[2]
				else:
					myrepeatdict['left'] = 100

				if len(fastadict[info[0]][rightseq[0]:rightseq[1]]) >= 100:
					myrepeatdict['right'] = rightseq[2]
				else:
					myrepeatdict['right'] = 100
				...
				print myrepeatdict
				print min(myrepeatdict, key=myrepeatdict.get)
				if os.path.getsize(SampleID+'_trf_left.output') == 0 and len(fastadict[info[0]][leftseq[0]:leftseq[1]]) >= 100:
					...
					print start
					print end
				elif min(myrepeatdict, key=myrepeatdict.get) == 'orig' and len(fastadict[info[0]][start:end]) >= 100:
					start = start
					end = end
					print 'origseq'
					print start
					print end
				elif min(myrepeatdict, key=myrepeatdict.get) == 'left' and len(fastadict[info[0]][leftseq[0]:leftseq[1]]) >= 100:
					start = leftseq[0]
					end = leftseq[1]
					print 'leftseq'
					print start
					print end
				elif min(myrepeatdict, key=myrepeatdict.get) == 'right' and len(fastadict[info[0]][rightseq[0]:rightseq[1]]) >= 100:
					start = rightseq[0]
					end = rightseq[1]
					print 'rightseq'
					print start
...
os.system('/home/phuong/bbmap/filterbyname.sh in=/pylon2/bi4s86p/phuong/venom/fastqfiles/' + SampleID + '_final1.fq in2=/pylon2/bi4s86p/phuong/venom/fastqfiles/' + SampleID + '_final2.fq out='+ SampleID + '_venomreads1.fq out2='+ SampleID + '_venomreads2.fq names=' + SampleID + '_venom_read_IDs substring=f include=t ow=t')
os.system('/home/phuong/bbmap/filterbyname.sh in=/pylon2/bi4s86p/phuong/venom/fastqfiles/' + SampleID + '_finalunpaired.fq out='+ SampleID + '_venomreadsunpaired.fq names=' + SampleID + '_venom_read_IDs substring=f include=t ow=t')
