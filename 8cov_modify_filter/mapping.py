...
#	python makeVCFcov.py {ref} {sample}_dir8.vcf {sample}_dir8.cov
	commands = """
	...
	java -jar $PICARD_HOME/picard.jar MarkDuplicates INPUT={outfile}.bam OUTPUT={sample}_md_dir8.bam REMOVE_DUPLICATES=TRUE ASSUME_SORTED=TRUE METRICS_FILE={sample}_md_dir8.metrics
	samtools index {sample}_md_dir8.bam
	samtools mpileup -d 1000000 -u -I -t DP -t SP -B -A -f {ref} {sample}_md_dir8.bam | /home/phuong/bcftools-1.3.1/bcftools call -c - > {sample}_dir8.vcf
	rm {out_paired}.sam {out_paired}.bam {out_unpaired}.sam {out_unpaired}.bam {sample}.raw.bam
	python get_alternative_alleles.py {sample} {sample}_dir8.vcf
	python refine_sequences.py {sample}
	""".format(**variables)
